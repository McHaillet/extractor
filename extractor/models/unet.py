...
class UNet3D(nn.Module):
    def __init__(self, in_channels: int, out_channels: int,
                 filters: tuple[int, ...] = (32, 64, 128),  # reduced params, before was: (64, 128, 256, 512, 1024)
...
class Center3D(nn.Module):
    def __init__(self, in_channels, middle_channels, out_channels, deconv_channels, dropout=False):
        ...
        layers = [
            ...
            nn.BatchNorm3d(out_channels),
            nn.ReLU(inplace=True),
            nn.ConvTranspose3d(out_channels, deconv_channels, kernel_size=(2, ) * 3, stride=(2, ) * 3)
        ]
...
class Last3D(nn.Module):
    def __init__(self, in_channels, middle_channels, out_channels):
        ...
        layers = [
            ...
            nn.BatchNorm3d(middle_channels),
            nn.ReLU(inplace=True),
            nn.Conv3d(middle_channels, middle_channels, kernel_size=(3, ) * 3, padding=1),
            nn.BatchNorm3d(middle_channels),
            nn.ReLU(inplace=True),
            nn.Conv3d(middle_channels, out_channels, kernel_size=(1, ) * 3)
        ]
